...
public class FileSystemStorageService implements StorageService {
    ...
    private List<byte[]> convertImagesToByteArrays(List<Image> images) {
        return images.stream().map(image -> {
            try {
                InputStream in = getClass().getClassLoader()
                        .getResourceAsStream("static/images/" + image.getId() + "." + image.getExtension());
                return IOUtils.toByteArray(in);
            } catch (IOException e) {
                e.printStackTrace();
                throw new RuntimeException("There was an error converting image " + image.getId() + " to byte array!");
            }
    ...
    private BufferedImage getBufferedImage(MultipartFile file) {
        ...
        try (InputStream in = new ByteArrayInputStream(file.getBytes())) {
            ...
            if (bufferedImage == null)
                throw new RuntimeException("There was an error buffering image!");
